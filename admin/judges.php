...
require ('check.inc');
require('../connect.php');
if ($_POST['delid']) {mysql_query('delete from mark where njudge='.$_POST['delid']);mysql_query('delete from judges where id='.$_POST['delid']);}
if ($_POST['add'])
...
{for($i=0;$i<count($_POST['id']);$i++)
 {$q='';
  if ($_POST['cname_local'][$i]) {$q.=',cname_local="'.$_POST['cname_local'][$i].'"';}
