...
function qt($st) {$st=trim($st,'"'); if ($st) return '"'.$st.'"'; else return 'NULL';}
function qc($st) {$st=trim($st,'"'); if ($st) return 'coalesce((select ncode from flags where lower(cvalue)=lower("'.$st.'") or lower(c3let)=lower("'.$st.'") or lower(c2let)=lower("'.$st.'") limit 1),(select nflag from flags_syn where cvalue="'.$st.'" order by id desc limit 1))'; else return 'NULL';}

function qd($st) {if (trim($st,'"')) return 'str_to_date("'.trim($st,'"').'", "%d.%m.%Y")'; else return 'NULL';}
